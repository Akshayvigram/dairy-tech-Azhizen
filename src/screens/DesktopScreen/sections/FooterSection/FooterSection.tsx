...
export const FooterSection = ({}: FooterSectionProps): JSX.Element => {
  ...
  // 📍 Exact route match with App.tsx: "/services"
  const SERVICES_ROUTE = "/services";

  const quickLinks = [
    { label: "Home", type: "scroll", id: "home" },
    { label: "Service", type: "link", path: SERVICES_ROUTE },
    { label: "Shop", type: "link", path: "/shop" },
  ...
  const socialLinks = [
    ...
    {
      label: "Instagram",
      href: "https://www.instagram.com/azhizensolutions?igsh=YTQ1cTdheTFjM3py",
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
  ...
  const servicesList = [
    { label: "Dairy Tech Service" },
    { label: "Goat Farm Service" },
    { label: "Poultry Farm Service" },
  ];
  ...
  return (
    <footer id="contact" className="w-full bg-black py-8 sm:py-12 md:py-16 relative z-30">
      <div className="max-w-[1280px] mx-auto px-6 md:px-12 w-full">
        {/* Grid: 1 column on mobile, 5 separate columns on desktop */}
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 md:gap-10 mb-8 md:mb-12">

          {/* Col 1: Brand Info & Social Icons */}
          <div className="flex flex-col gap-4">
            <h2 className="font-sans font-bold text-white text-[18px] sm:text-[22px] md:text-[24px] tracking-[0] leading-tight">
              AZHIZEN SOLUTION
            </h2>
            <p className="font-sans font-normal text-white text-[11px] sm:text-[12px] md:text-[13px] tracking-[0] leading-relaxed max-w-[271px]">
              Easemilker Is An Automated Smart Milking System That Ensures
              Hygienic, Efficient, And Stress-free Milking For Cows.
            </p>
            ...
            <div className="flex gap-4 items-center pt-2">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="text-[#ddd3ba] hover:text-white transition-colors w-5 h-5 flex items-center justify-center transform"
                  title={social.label}
                >
                  {social.icon}
                </a>
          ...
          {/* Col 2: Quick Links */}
          <div className="flex flex-col gap-4">
            <h3 className="font-sans font-bold text-white text-[16px] sm:text-[18px] md:text-[20px] tracking-[0] leading-tight">
              Quick Links
            </h3>
            <nav className="flex flex-col gap-2.5">
              {quickLinks.map((link) =>
                link.type === "scroll" ? (
                  <button
                    key={`quick-link-${link.label}`}
                    type="button"
                    onClick={handleHomeScroll}
                    className="text-left font-sans font-normal text-[#ddd3ba] text-[12px] md:text-[13.2px] hover:text-white transition-colors cursor-pointer"
                  >
                    {link.label}
                  </button>
                ) : (
            ...
            <nav className="flex flex-col gap-2.5">
              {products.map((product, index) => (
                <Link
                  ...
                  to="/shop"
                  onClick={() => window.scrollTo(0, 0)}
                  className="text-left font-sans font-normal text-[#ddd3ba] text-[12px] md:text-[13.2px] hover:text-white transition-colors cursor-pointer whitespace-nowrap"
                >
                  {product.label}
                </Link>
              ))}
          ...
          {/* Col 4: Services (Direct Link to /services Route) */}
          <div className="flex flex-col gap-4">
            <h3 className="font-sans font-bold text-white text-[16px] sm:text-[18px] md:text-[20px] tracking-[0] leading-tight">
              Services
            </h3>
            <nav className="flex flex-col gap-2.5">
              {servicesList.map((service, index) => (
                <Link
                  ...
                  to={SERVICES_ROUTE}
                  onClick={() => window.scrollTo(0, 0)}
                  className="text-left font-sans font-normal text-[#ddd3ba] text-[12px] md:text-[13.2px] hover:text-white transition-colors cursor-pointer whitespace-nowrap block"
                >
                  {service.label}
                </Link>
              ))}
              ...
              {contactInfo.map((contact, index) => (
                ...
                >
                  <img
                    ...
                    src={contact.icon}
                  />
                  <span className="font-sans font-normal text-[#ddd3ba] text-[11px] sm:text-[12px] md:text-[13.2px] tracking-[0] leading-normal break-words">
                    {contact.text}
                  </span>
                </div>
              ))}
        ...
        <div className="text-center">
          <p className="font-sans font-normal text-white text-[12px] sm:text-[13.2px] tracking-[0] leading-normal">
