...
interface TechTransitionProps {
  onComplete: () => void;
  direction?: 'to-tech' | 'to-dairy'; // Default: 'to-tech'
}

export const TechTransition: React.FC<TechTransitionProps> = ({
  onComplete,
  direction = 'to-tech',
}) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  ...
  useEffect(() => {
    ...
    function buildLiquid(
      ...
      // Layer 1: Darkest Background Wave
      wctx.beginPath();
      wctx.moveTo(0, H);
      for (let x = 0; x <= W; x++) {
        const y = fillY + Math.sin(x * 0.01 + waveOffset * 0.04) * 15;
    ...
    function start() {
      ...
      // Set initial (draining) and target (filling) themes based on direction
      const fromTheme = direction === 'to-tech' ? greenTheme : blueTheme;
      const toTheme = direction === 'to-tech' ? blueTheme : greenTheme;
      ...
      function drawFrame() {
        ...
        if (!isFlipping) {
          // PHASE 1: DRAINING DOWN
          progress += 0.012;
          const fillY = progress * H;

          const liquid = buildLiquid(W, H, fillY, waveOffset, fromTheme.colors);
          const masked = maskLiquid(fromTheme.img, liquid, W, H);

          ctx.globalAlpha = 0.05;
          ctx.drawImage(fromTheme.img, 0, 0, W, H);
          ctx.globalAlpha = 1;
          ctx.drawImage(masked, 0, 0);
          ...
          const liquid = buildLiquid(W, H, fillY, waveOffset, toTheme.colors);
          const masked = maskLiquid(toTheme.img, liquid, W, H);

          ctx.globalAlpha = 0.05;
          ctx.drawImage(toTheme.img, 0, 0, W, H);
          ctx.globalAlpha = 1;
          ctx.drawImage(masked, 0, 0);
  ...
  const isToTech = direction === 'to-tech';
      ...
      <div className="mt-10 flex flex-col items-center gap-2">
        <div
          className={`w-6 h-6 border-2 border-t-transparent rounded-full animate-spin ${
            isToTech ? 'border-blue-400' : 'border-[#7CB142]'
          }`}
        />
        <p className="text-gray-400 font-medium tracking-widest text-[10px] uppercase font-[Poppins]">
          {isToTech ? 'Switching to Azhizen Tech' : 'Switching to Dairy Tech'}
        </p>
      </div>
    </div>
  );
